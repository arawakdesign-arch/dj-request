...
function editorialChrome(doc, page, label) {
  ...
  doc.font('Helvetica-Bold').fontSize(7).fillColor('#c8c2cb').text('PULL UP!  /  ARTIST PRESS KIT', 24, 420, { characterSpacing: 2 });
  doc.restore();
...
function footer(doc, profileUrl) {
  doc.save().moveTo(38, 789).lineTo(337, 789).lineWidth(0.7).strokeColor('#746d78').stroke().restore();
  doc.font('Helvetica-Bold').fontSize(7).fillColor('#fff').text(text(profileUrl, 90), 38, 806, { width: 295 });
  doc.font('Helvetica-Bold').fontSize(7).fillColor('#fff').text('CRÉE TON PRESS KIT', 342, 806, { width: 118, align: 'right', characterSpacing: 0.8 });
  if (fs.existsSync(logoPath)) doc.image(logoPath, 469, 754, { fit: [84, 84] });
}
...
function drawCover(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(9).fillColor('#fff').text('PULL UP! ARTISTS', 377, 41, { width: 180, align: 'right', characterSpacing: 1.3 });
...
function drawAbout(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('ABOUT ME', 39, 40, { characterSpacing: 1.8 });
  const name = text(profile.stage_name, 60).toUpperCase() || 'ARTISTE';
  ...
  doc.font('Helvetica-Bold').fontSize(13).fillColor('#d7c5f5').text('BIOGRAPHIE', 39, 239, { characterSpacing: 1.2 });
  const bio = text(profile.bio, 5000) || 'Présentation à venir.';
  const bioSize = bio.length > 4000 ? 8 : bio.length > 2800 ? 9 : bio.length > 1700 ? 10.5 : bio.length > 900 ? 12.5 : 15;
  const bioColumns = bio.length > 2000 ? 2 : 1;
  doc.font('Helvetica').fontSize(bioSize).fillColor('#fff').text(bio, 39, 278, { width: 516, height: 407, columns: bioColumns, columnGap: 30, lineGap: bioColumns === 1 ? 6 : 4, ellipsis: true });
  doc.save().fillOpacity(0.16).roundedRect(38, 704, 517, 66, 5).fill('#ffffff').restore();
  doc.font('Helvetica-Bold').fontSize(8).fillColor('#cdb5f5').text('SIGNATURE ARTISTIQUE', 51, 717, { characterSpacing: 1.1 });
  doc.font('Helvetica').fontSize(10).fillColor('#fff').text(text(profile.tagline, 150), 51, 741, { width: 480, height: 20, ellipsis: true });
  footer(doc, profileUrl);
}
...
function drawMusic(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('MUSIC / SERVICES', 39, 40, { characterSpacing: 1.8 });
  ...
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#cdb5f5').text('STYLES', 39, 190, { characterSpacing: 1.3 });
  ...
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#cdb5f5').text('PRESTATIONS', 39, 335, { characterSpacing: 1.3 });
  ...
  const linkTop = 548;
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#cdb5f5').text('ÉCOUTER / REGARDER', 39, linkTop - 2, { characterSpacing: 1.3 });
  ...
  doc.font('Helvetica-Bold').fontSize(8).fillColor('#cdb5f5').text('IDENTITÉ MUSICALE', 52, 708, { characterSpacing: 1.1 });
...
function drawExperience(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('STORY / COLLABORATIONS', 39, 40, { characterSpacing: 1.8 });
  ...
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#cdb5f5').text('AUTRES COLLABORATIONS ET PRÉCISIONS', 39, 190, { characterSpacing: 1.2 });
  if (experience) {
  ...
  if (selected.length) {
    ...
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#cdb5f5').text('LIEUX MARQUANTS', 52, 699, { characterSpacing: 1.1 });
...
function drawCareer(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('CAREER / PLACES', 39, 40, { characterSpacing: 1.8 });
  ...
  if (!career.length) {
    ...
    career.forEach((entry, index) => {
      ...
      doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#cdb5f5').text(text(entry.type, 30).toUpperCase() || 'EXPÉRIENCE', x + 14, y + 12, { width: 145, characterSpacing: 0.8 });
...
function drawEvents(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('UPCOMING EVENTS', 39, 40, { characterSpacing: 1.8 });
  ...
  if (!events.length) {
    ...
    events.forEach((event, index) => {
      ...
      doc.font('Helvetica-Bold').fontSize(8).fillColor('#cdb5f5').text(text(event.date, 10) || 'DATE À VENIR', x + 13, y + 149, { characterSpacing: 0.7 });
...
function drawMedia(doc, profile, assets, profileUrl) {
  ...
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#fff').text('MEDIA / CONTACT', 39, 40, { characterSpacing: 1.8 });
  ...
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#cdb5f5').text('BOOKING', 54, 540, { characterSpacing: 1.2 });
  ...
  bookingRows.forEach(([label, value], index) => {
    ...
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#bdb5c1').text(label, 54, y - 1, { characterSpacing: 0.7 });
  ...
  doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#bdb5c1').text('PAGE PUBLIQUE', 54, 718, { characterSpacing: 0.7 });
  ...
  if (assets.qr) {
    ...
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#fff').text('SCANNER LE PROFIL', 421, 674, { width: 122, align: 'center', characterSpacing: 0.7 });
  }
