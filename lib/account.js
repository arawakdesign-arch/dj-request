...
async function deleteUserAccount(uid) {
  ...
  const [profile, djProfile, orgaPage, ownMessages] = await Promise.all([
    ...
    // gallery: jusqu'à 6 photos supplémentaires uploadées séparément (cf.
    // POST /dj/profile/gallery) — un oubli ici laissait ces fichiers orphelins
    // dans le stockage après suppression du compte.
    read('dj_profiles',     supabase.from('dj_profiles').select('*').eq('id', uid).maybeSingle()),
  ...
  const filesToRemove = [
    ...
    ['profile-photos', djProfile?.presskit_pdf_url],
    ['orga-logos',     orgaPage?.logo_url],
    ['orga-logos',     orgaPage?.banner_url],
    ...(ownMessages || []).map(m => ['chat-photos', m.photo_url]),
    ...(djProfile?.gallery || []).map(url => ['profile-photos', url]),
    ...((djProfile?.upcoming_events || []).map(event => ['profile-photos', event?.flyer_url])),
  ];
