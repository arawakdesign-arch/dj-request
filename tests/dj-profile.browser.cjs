...
(async()=>{
 ...
 assert.equal(await page.locator('.djr-pdf-btn').innerText(),'Télécharger mon press kit PDF');
 ...
 await page.evaluate(()=>{window.__pressKitPrinted=false;window.print=()=>{window.__pressKitPrinted=true;};});
 await page.locator('.djr-pdf-btn').click();
 await page.waitForFunction(()=>window.__pressKitPrinted===true);
 assert.equal(await page.locator('#pg-presskit.active').count(),1);
 await page.evaluate(()=>openDjRegister(true));
 await page.locator('#djr-editor').waitFor({state:'visible'});
