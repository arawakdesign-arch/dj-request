...
function renderClient() {
  ...
  if (!s.length) { list.innerHTML = `<div class="empty-state" style="padding:.5rem 1rem 6rem"><img src="/images/proposeimagedefond.png" alt="Le classement commence ici — propose un titre, les participants voteront pour le faire monter" style="width:100%;max-width:360px;height:auto;display:block"></div>`; return; }
  ...
  s.forEach((p, i) => {
    ...
    if (isNew) {
      ...
      div.innerHTML = `
        ...
        <button class="vote-pill" onclick="vote('${p.id}')" style="${eventClosed?'opacity:.5':''}">${eventClosed?'Clos':(voted?'GO ✓':'VOTER ›')}</button>`;
    } else {
      ...
      btn.style.opacity  = eventClosed ? '.5' : '';
      btn.textContent    = eventClosed ? 'Clos' : (voted ? 'GO ✓' : 'VOTER ›');
    }
...
// Lien court à partager (QR code, bouton Copier, invitation) — droit vers
// la soirée en cours, pas vers la page publique de l'organisateur : le but
// est de voter tout de suite, un détour par la page organisateur (qui liste
// toutes ses soirées) ajoute juste un clic en trop pour rien.
